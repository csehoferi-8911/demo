...
class OmdbApiServiceTest {
    ...
    void testGetMovieDetails_Success() {
        ...
        assertEquals(MovieUtil.MOVIE_TITLE, result.movieResponses().getFirst().title());
        ...
        verify(redisService, times(1)).saveToCache(movieResponse,  MovieUtil.OMDB_API_NAME);
    }
    ...
    void testGetMovieDetails_EmptyResponse() {
        ...
        assertEquals(0, result.movieResponses().size());

        verify(movieRepository, never()).save(any());
        verify(redisService, never()).saveToCache(any(), anyString());
    }
}
